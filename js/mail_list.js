    ...
    for(var i = 0; i < motherlist.length; ++i){
        ...
        for(var j = 0; j < tmp.length; ++j){
            if(tmp[j][4] == motherlist[i]){
                ...
                parent_name = tmp[j][4];
                mail_address = tmp[j][5];
                far = tmp[j][3];
                brother = tmp[j][6];
            }
        }
        memberList.push({"namelist":namelist, "parent_name":parent_name, "mail_address":mail_address, "far":far, "brother":brother});
    }
...
window.onload = function(){
    ...
    for (var i = 0; i < members.length; ++i){
        ...
        contents = contents.replace( /#PARENT_NAME#/g , members[i]["parent_name"]);
        contents = contents.replace( /#MAIL_ADDRESS#/g , members[i]["mail_address"]);
